...
class PROBLEM(MultiObjectiveTestProblem, ConstrainedBaseTestProblem):
    ...
    def evaluate_slack_true(self, X: Tensor) -> Tensor:
        ...
        T_store[..., 1:] = T_store[..., :-1] + (sol_transf - Pth_direct - (hp_out - hp_in)
                                - Ui * (T_store[..., :-1] - Ta[:Horizon]) * (np.pi * (D_array[2] / 2)**2) / 1000
                                - Ui * ((T_store[..., :-1] - 12) * np.pi * (D_array[2] / 2) * D_array[2] * 0.1) / 1000
                                - k_gr * ((T_store[..., :-1] - 12) * (D_array[2] / 2) * h) / 1000) * (dt / th_cap[0])

        st_g1 = dm * (UA[2] * DTd + hp_in) - hp_out
        st_g3 = (1 - dm) * UA[2] * (T_store[..., :-1] - (system_Ts_h - DTnet)) - Pth_direct
        st_g4 = UA[2] * (70 - T_store[..., :-1]) - sol_transf
...
train_obj.to_csv('obj_config3.csv', index=False)
train_x = pd.DataFrame(train_x_qnehvi)
train_x.to_csv('x_config3.csv', index=False)
hvs = pd.DataFrame(hvs_qnehvi)
hvs.to_csv('hv_config3.csv', index=False)
