...
class PROBLEM(MultiObjectiveTestProblem, ConstrainedBaseTestProblem):
    ...
    upper_bounds_array = sol_eff * pv_irradiation * sol_cap_max
    ...
    def evaluate_slack_true(self, X: Tensor) -> Tensor:
        ...
        eps = 0.1
        # solar
        sol_out = sol_eff * pv_irradiation * sol_cap.unsqueeze(1)
        sol_g1 = sol_eff * pv_irradiation * sol_cap_max - sol_out
        
        sol_inv = sol_cap  * sol_capital + sol_cap * max_sol_gen * sol_tank_capital
        sol_g2 = sol_cap_max  * sol_capital + sol_cap_max * max_sol_gen * sol_tank_capital - sol_inv
        ...
        T_store[..., 1:] = T_store[..., :-1] + (sol_transf - Pth_direct - (hp_out - hp_in)
                                - Ui * (T_store[..., :-1] - Ta[:Horizon]) * (np.pi * (D_array[0] / 2)**2) / 1000
                                - Ui * ((T_store[..., :-1] - 12) * np.pi * (D_array[0] / 2) * D_array[0] * 0.1) / 1000
                                - k_gr * ((T_store[..., :-1] - 12) * (D_array[0] / 2) * h) / 1000) * (dt / th_cap[0])
   
        st_g1 = dm * (UA[0] * DTd + hp_in) - hp_out
        st_g3 = (1 - dm) * UA[0] * (T_store[..., :-1] - (system_Ts_h - DTnet)) - Pth_direct
        st_g4 = UA[0] * (70 - T_store[..., :-1]) - sol_transf
...
train_obj.to_csv('obj_config1.csv', index=False)
train_x = pd.DataFrame(train_x_qnehvi)
train_x.to_csv('x_config1.csv', index=False)
hvs = pd.DataFrame(hvs_qnehvi)
hvs.to_csv('hv_config1.csv', index=False)
